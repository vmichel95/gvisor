...
func (h *virtioNetHdr) marshal() []byte {
	...
	buf[2] = byte(h.hdrLen)
	buf[3] = byte(h.hdrLen >> 8)

	buf[4] = byte(h.gsoSize)
	buf[5] = byte(h.gsoSize >> 8)

	buf[6] = byte(h.csumStart)
	buf[7] = byte(h.csumStart >> 8)

	buf[8] = byte(h.csumOffset)
	buf[9] = byte(h.csumOffset >> 8)
