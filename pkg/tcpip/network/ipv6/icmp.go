...
func (e *endpoint) handleICMP(pkt *stack.PacketBuffer, hasFragmentHeader bool) {
	...
	case header.ICMPv6NeighborSolicit:
		...
		if len(sourceLinkAddr) == 0 {
			...
			e.nud.HandleProbe(srcAddr, dstAddr, header.IPv6ProtocolNumber, sourceLinkAddr, e.protocol)
		} else {
	...
	case header.ICMPv6RouterSolicit:
		...
		if len(sourceLinkAddr) != 0 {
			...
			if e.nud != nil {
				...
				e.nud.HandleProbe(srcAddr, dstAddr, ProtocolNumber, sourceLinkAddr, e.protocol)
			}
		}
	...
	case header.ICMPv6RouterAdvert:
		...
		if len(sourceLinkAddr) != 0 && e.nud != nil {
			e.nud.HandleProbe(routerAddr, dstAddr, ProtocolNumber, sourceLinkAddr, e.protocol)
		}
