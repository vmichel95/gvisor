...
type lossRecovery interface {
	// Update is invoked to update variables in loss recovery algorithm.
	Update()

	// DoRecovery is invoked when loss is detected and segments need to be
	// retransmitted.
	DoRecovery(seg *segment, rtx bool)
}
...
func (s *sender) enterRecovery() {
	...
	// Update depending on the the loss recovery algorithm used by sender.
	s.lr.Update()
}
...
// detectLoss is called when an ack is received. It manages the state
// related to duplicate acks and determines if a retransmit is needed according
// to the rules in RFC 6582 (NewReno).
func (s *sender) detectLoss(seg *segment) (rtx bool) {
	ack := seg.ackNumber
