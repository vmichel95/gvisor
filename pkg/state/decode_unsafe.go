...
func reflectValueRWSlice3(arr reflect.Value, i, j, k int) reflect.Value {
	...
	if i == arr.Len() {
		...
		return reflect.MakeSlice(reflect.SliceOf(elemTyp), 0, 0)
	}
	slh := reflect.SliceHeader{
		...
		Data: arr.Index(0).UnsafeAddr() + uintptr(i)*elemTyp.Size(),
		Len:  j - i,
		Cap:  k - i,
	}
	slobj := reflect.NewAt(reflect.SliceOf(elemTyp), unsafe.Pointer(&slh)).Elem()
