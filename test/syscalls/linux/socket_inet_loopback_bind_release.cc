...
// Global mutex protecting racy bind calls. The pattern for many tests below
// is to bind to an address/port, release it, and then bind to it again. This
// causes flakes as other bind calls may bind to the releaseed address/port.
// Treat the port space as a shared resource and protect it with this mutex.
absl::Mutex mutex;
