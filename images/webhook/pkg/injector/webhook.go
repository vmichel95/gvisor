...
func CreateConfiguration(clientset kubeclientset.Interface, selector *metav1.LabelSelector) error {
	...
	logrus.Infof("Creating MutatingWebhookConfiguration %q", config.Name)
	if _, err := clientset.AdmissionregistrationV1beta1().MutatingWebhookConfigurations().Create(config); err != nil {
		if !apierrors.IsAlreadyExists(err) {
			return errors.Errorf("Failed to create MutatingWebhookConfiguration %q: %s", config.Name, err)
		}
		logrus.Infof("MutatingWebhookConfiguration %q already exists; use the existing one", config.Name)
	}
	return nil
}
...
func GetTLSConfig() *tls.Config {
	...
	if err != nil {
		logrus.Fatalf("Failed to generate X509 key pair: %v", err)
	}
	return &tls.Config{
...
func Admit(writer http.ResponseWriter, req *http.Request) {
	...
	if err := json.NewDecoder(req.Body).Decode(review); err != nil {
		logrus.WithError(err).Infof("Failed to decode Admit request: %+v", *req)
		writer.WriteHeader(http.StatusBadRequest)
		return
	}

	logrus.Debugf("admitPod: %+v", review)
	var err error
	review.Response, err = admitPod(review.Request)
	if err != nil {
		logrus.Warnf("admitPod failed: %v", err)
	...
	logrus.Debugf("Processed admission review: %+v", review)
	sendResponse(writer, review)
}
...
func sendResponse(writer http.ResponseWriter, response interface{}) {
	...
	if err != nil {
		logrus.WithError(err).Errorf("Failed to marshal response: %+v", response)
...
func admitPod(req *admv1beta1.AdmissionRequest) (*admv1beta1.AdmissionResponse, error) {
	...
	logrus.Debugf("Patched pod %s/%s (generateName: %s): %+v", pod.Namespace, pod.Name, pod.GenerateName, podCopy)
